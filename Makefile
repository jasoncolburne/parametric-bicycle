...
# =============================================================================
# Plastic-CF Components (generated from assembly, no individual files)
# =============================================================================
PLASTIC_STL :=
...
$(STL_DIR)/metal/%.stl: $(COMPONENTS_DIR)/%.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ $<

# Dropout junctions (left and right)
$(STL_DIR)/metal/dropout_junction_left.stl: $(COMPONENTS_DIR)/dropout_junction.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_side=-1' $<

$(STL_DIR)/metal/dropout_junction_right.stl: $(COMPONENTS_DIR)/dropout_junction.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_side=1' $<

# Tube sleeves (all straight tubes)
$(STL_DIR)/metal/sleeve_down_tube.stl: $(COMPONENTS_DIR)/tube_sleeve.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_type="down_tube"' $<

$(STL_DIR)/metal/sleeve_seat_tube.stl: $(COMPONENTS_DIR)/tube_sleeve.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_type="seat_tube"' $<

$(STL_DIR)/metal/sleeve_chainstay.stl: $(COMPONENTS_DIR)/tube_sleeve.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_type="chainstay"' $<

$(STL_DIR)/metal/sleeve_seat_stay.stl: $(COMPONENTS_DIR)/tube_sleeve.scad $(CONFIG) | $(STL_DIR)/metal
	$(OPENSCAD) $(OPENSCAD_FLAGS) -o $@ -D 'render_type="seat_stay"' $<

$(STL_DIR)/metal/sleeve_top_tube.stl: $(COMPONENTS_DIR)/tube_sleeve.scad $(CONFIG) | $(STL_DIR)/metal
